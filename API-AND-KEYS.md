...
**На проде — вкладка Sandbox в админке** (пункт [admin-sandbox]
2026-08-31, нужна роль `isOperator`). Продовый API закрыт
Telegram-авторизацией, поэтому curl-проверки ниже там не работают —
Sandbox делает то же самое из браузера: чек-лист готовности всех
переменных этой цепочки (без показа значений), реальный YouTube-поиск,
прогон транскрибации на синтетическом WAV (проверяет ключ AssemblyAI,
загрузку, адрес и секрет вебхука, запись статусов — транскрипт
ожидаемо пустой) и запуск трёх видов анализа. Всё выполняется от имени
самого оператора, с его согласиями и его лимитами — песочница не
обходит ни одной проверки.

**Локально/в докере — curl с дев-входом:**
