...
| `AI_MEDIA_CALLS_PER_USER_PER_DAY` | отдельный, более низкий суточный потолок для МЕДИА-задач (видео/аудио через Gemini: media-public-review, паралингвистика) — один такой вызов стоит на порядки дороже текстового; счёт по taskType запроса; 429 при превышении; 0 = выключено (аудит 2026-09-02) | опциональна, дефолт 20 |
| `TTS_CALLS_PER_USER_PER_DAY` | суточный потолок синтеза речи на пользователя (считаются только cache-miss — реальный расход ElevenLabs; 0 = выключено) | опциональна, дефолт 100 |
...
## Soniox — распознавание русского и украинского (Пункт [stt-multi] 2026-09-02)

`SONIOX_API_KEY` — soniox.com → API keys.

Зачем отдельный вендор: у AssemblyAI НЕТ русского и украинского ни в
одной потоковой модели (живой прогон голосового ввода на русском вернул
галлюцинацию на английском и иврите), а в файлах они есть только у
universal-2 с точностью категории «good». Soniox ведёт ru/uk и в живом
режиме, и в файлах, одной мультиязычной моделью с переключением языка
внутри фразы; диаризация и определение языка входят в цену.

Кто что обслуживает: **ru/uk и неизвестный язык — Soniox, английский —
AssemblyAI, общий фоллбек короткой записи — ElevenLabs** (ключ у него
уже есть, он же озвучивает реплики). Разбор и цены —
`docs/devils-advocate-stt-provider-2026-09-02.md`.

Без ключа Soniox: живой режим для ru/uk недоступен, короткая запись
уходит в ElevenLabs, длинный файл — в AssemblyAI. То есть деградация, а
не отказ, — но качество на ru/uk будет прежним.
