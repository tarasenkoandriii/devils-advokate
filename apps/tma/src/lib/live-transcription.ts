// Пункт 82 (§3.4 ТЗ) → клиентское подключение к сервису распознавания
// НАПРЯМУЮ из браузера, по короткоживущему токену из
// mintTranscriptionToken() (Пункт 81).
//
// Пункт [stt-multi] 2026-09-02 — ПРОВАЙДЕРОВ ДВА, и выбирает их язык
// пользователя (решение приходит с сервера вместе с токеном):
//
//   • Soniox (wss://stt-rt.soniox.com/transcribe-websocket) — русский,
//     украинский и их смесь. Причина: у AssemblyAI НЕТ ru/uk ни в одной
//     потоковой модели, и живой прогон на русском возвращал
//     галлюцинацию на английском и иврите;
//   • AssemblyAI Streaming v3 — английский, прежний путь без изменений.
//
// Протоколы разные (у Soniox конфигурация уходит первым сообщением, у
// AssemblyAI — параметрами URL), поэтому здесь два транспорта за одним
// интерфейсом; вызывающие экраны о различии не знают.
//
...
/**
 * Подключение к живой расшифровке. Принимает либо реквизиты от
 * бэкенда (новый путь, провайдер выбран по языку), либо голый токен —
 * прежняя сигнатура, оставленная ради вызывающих, которые ещё не
 * обновлены: голый токен означает AssemblyAI, как и раньше.
 */
