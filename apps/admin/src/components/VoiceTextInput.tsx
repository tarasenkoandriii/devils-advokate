...
//
// Поэтому два транспорта под одним селектором языка (состояние ПОСЛЕ
// Пункта [stt-multi] 2026-09-02 — шапка обновлена аудитом того же дня,
// прежняя описывала уже не действующую схему):
// - ru/uk → живой стриминг в Soniox (ru и uk у него есть в реальном
//   времени, переключение языка внутри фразы — тоже);
// - en → живой стриминг в AssemblyAI как раньше, с ПИНОМ языка
//   (language_codes=["en"]) — без пина модель угадывает язык по звуку;
// - auto → запись MediaRecorder'ом и синхронная транскрипция короткой
//   заметки (SttService.transcribeSync: Soniox, при отказе ElevenLabs):
//   текст приходит один раз по кнопке «Стоп» — без выбранного языка
//   честнее дать модели весь файл, чем угадывать провайдера на лету.
// Аудио в обоих путях не проходит транзитом никуда, кроме провайдера
// распознавания, и нигде не сохраняется (заметка — буфер в пределах
// одного запроса; у Soniox копия удаляется сразу после результата).
