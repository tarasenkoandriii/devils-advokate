...
-- не затрагивается.
--
-- ПОРЯДОК (важен весь, включая шаг с генерацией клиента):
--
--   1) применить ЭТОТ файл к базе — обязательно по DIRECT_URL (порт
--      5432): DDL не проходит через pgbouncer.
--        psql "$DIRECT_URL" -f prisma/manual-migrations/router_simplify_2026_09_01.sql
--   2) npx prisma generate   — иначе сгенерированный клиент остаётся со
--      старой схемой, и `prisma db seed` падает на КОМПИЛЯЦИИ seed.ts
--      («Property 'taskType' is missing…»), не дойдя до базы. Скрипт
--      npm run prisma:seed делает этот шаг сам.
--   3) npm run prisma:seed --workspace=apps/api
--
-- НЕ ЗАМЕНЯТЬ ЭТОТ ФАЙЛ НА `prisma db push`: push попытается создать
-- unique-индекс на modelVersionId поверх 192 строк с дубликатами по
-- этой колонке и упадёт (а до того предложит удалить колонки без
-- схлопывания). Схлопывание строк — шаг 1 ниже, и оно должно пройти
-- раньше индекса.
