...
// (яркие пиксели окантовки). Итоговые оттенки — чистые версии того
// же цветового семейства, не сырые сэмплированные значения (JPEG-
// сжатие даёт смещённые в серый оттенки).
//
// Пункт: после замены иллюстрации на новую версию (круги 4/5
// профессионально переставлены дизайнером) координаты пересчитаны
// заново под неё — старые (для предыдущей версии картинки) были
// заметно левее. В НОВОЙ версии иллюстрации пунктирной линии-
// "артефакта" под иконками больше нет вообще (упрощённый дизайн) —
// текст выровнен по центру самих иконок и поднят выше (близко к
// нижнему краю иконки, не в середину пустого пространства под ней).
//
// Первая попытка (18.4/32.6/46.2/60.1 не хватало точности слева,
// последние два — 72.4/83.8 — сразу легли верно) была откалибрована
// повторным замером с перекрёстной проверкой против уже подтверждённых
// правильными координат. top поднят слишком сильно в первой попытке
// (10%) — перекрывал кольца самих иконок, опущен обратно с запасом
// до 12.5%, затем по прямому запросу приподнят ещё на 5px (0.586% от
// высоты канваса 853px).
const PIPELINE_LEFT = [18.4, 32.6, 46.2, 60.1, 72.4, 83.8];
const PIPELINE_TOP = 11.91;
